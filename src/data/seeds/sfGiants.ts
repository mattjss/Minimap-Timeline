import type { TimelineEvent } from '../../types'
import { ev, evMinor } from '../seedBuilders'

const team: TimelineEvent['related'] = [
  { kind: 'team', label: 'San Francisco Giants' },
]

function heroMedia(id: string): TimelineEvent['media'] {
  return [
    {
      id: `${id}-hero`,
      type: 'image',
      alt: 'Historical photograph (placeholder)',
    },
  ]
}

function galleryStub(id: string): TimelineEvent['media'] {
  return [
    {
      id: `${id}-g1`,
      type: 'image',
      alt: 'Archive still 1 (placeholder)',
    },
    {
      id: `${id}-g2`,
      type: 'image',
      alt: 'Archive still 2 (placeholder)',
    },
  ]
}

/** Additional milestones: drafts, trades, ballparks, icons, records */
const SF_GIANTS_EXPANDED: TimelineEvent[] = [
  evMinor(
    'sf-giants-1959-mccovey-roy',
    1959,
    '11-15',
    'Willie McCovey wins NL Rookie of the Year',
    'Award',
    'Thunderous debut summer cements McCovey as a Bay Area folk hero.',
    { subtype: 'award', related: [...team, { kind: 'person', label: 'Willie McCovey' }] },
  ),
  evMinor(
    'sf-giants-1960-candlestick',
    1960,
    '04-12',
    'Candlestick Park opens for MLB',
    'Ballpark',
    'Wind, cold, and charm: the Giants leave Seals Stadium for the Stick.',
    { subtype: 'venue', related: [...team, { kind: 'place', label: 'Candlestick Park' }] },
  ),
  evMinor(
    'sf-giants-1960-marichal-debut',
    1960,
    '07-19',
    'Juan Marichal’s first start — a shutout',
    'Player',
    'The Dominican Dandy arrives with high kicks and complete-game magic.',
    { related: [...team, { kind: 'person', label: 'Juan Marichal' }] },
  ),
  ev(
    'sf-giants-1963-marichal-spahn',
    1963,
    '07-02',
    'Marichal vs. Spahn 16-inning duel',
    'Classic',
    'Two legends trade zeros until Willie Mays ends it with a walk-off.',
    {
      importance: 2,
      subtype: 'classic-game',
      related: [...team, { kind: 'person', label: 'Willie Mays' }],
    },
  ),
  evMinor(
    'sf-giants-1968-perry-no-no',
    1968,
    '09-17',
    'Gaylord Perry no-hitter',
    'Pitching',
    'Future Hall of Famer spins a zero on the Candlestick stage.',
    { related: [...team, { kind: 'person', label: 'Gaylord Perry' }] },
  ),
  evMinor(
    'sf-giants-1971-division',
    1971,
    '10-01',
    'First NL West division title',
    'Postseason',
    'Giants claim the inaugural NL West crown behind Mays, Bonds Sr., and pitching depth.',
    { related: team },
  ),
  ev(
    'sf-giants-1982-clark-draft',
    1982,
    '06-07',
    'Giants draft Will Clark',
    'Draft',
    '“Will the Thrill” becomes the emotional core of late-80s contenders.',
    {
      importance: 2,
      subtype: 'draft',
      related: [...team, { kind: 'person', label: 'Will Clark' }],
    },
  ),
  ev(
    'sf-giants-1987-pennant-race',
    1987,
    '09-30',
    '“Cinderella Story” pennant chase',
    'Culture',
    'Kruk & Kuip on the air; Clark’s slash fuels a summer that felt like October.',
    { importance: 2, related: [...team, { kind: 'person', label: 'Will Clark' }] },
  ),
  ev(
    'sf-giants-1992-bonds-signs',
    1992,
    '12-08',
    'Barry Bonds signs as a free agent',
    'Free agency',
    'The son of a Giants star returns to reshape the record books in orange and black.',
    {
      importance: 3,
      subtype: 'free-agency',
      richMedia: true,
      related: [...team, { kind: 'person', label: 'Barry Bonds' }],
    },
  ),
  ev(
    'sf-giants-1993-magowan-group',
    1993,
    '01-12',
    'Magowan-led group buys the Giants',
    'Ownership',
    'A Bay Area ownership team keeps the franchise from moving to Tampa.',
    {
      importance: 3,
      related: [...team, { kind: 'person', label: 'Peter Magowan' }],
    },
  ),
  evMinor(
    'sf-giants-1997-kent-trade',
    1997,
    '07-29',
    'Jeff Kent acquired in blockbuster',
    'Trade',
    'Kent and Bonds form the NL’s most feared middle-of-the-order tandem.',
    { subtype: 'trade', related: [...team, { kind: 'person', label: 'Jeff Kent' }] },
  ),
  evMinor(
    'sf-giants-1999-sabean-gm',
    1999,
    '09-30',
    'Brian Sabean named general manager',
    'Front office',
    'The architect behind Bonds-era contenders and the 2010s core takes the helm.',
    { related: [...team, { kind: 'person', label: 'Brian Sabean' }] },
  ),
  ev(
    'sf-giants-2002-bonds-obp',
    2002,
    '10-05',
    'Bonds walks into history (.582 OBP)',
    'Record',
    'Intentional walks become a strategy and a spectacle.',
    { importance: 2, subtype: 'record', related: [...team, { kind: 'person', label: 'Barry Bonds' }] },
  ),
  ev(
    'sf-giants-2004-bonds-232-walks',
    2004,
    '10-03',
    'Bonds draws 232 walks in a single season',
    'Record',
    'A staggering plate-discipline mark that may never be touched.',
    { importance: 2, subtype: 'record', related: [...team, { kind: 'person', label: 'Barry Bonds' }] },
  ),
  ev(
    'sf-giants-2007-bonds-756',
    2007,
    '08-07',
    'Barry Bonds hits home run no. 756',
    'Record',
    'McCovey Cove splashes as Bonds passes Aaron on the all-time list.',
    {
      importance: 3,
      subtype: 'record',
      richMedia: true,
      related: [...team, { kind: 'person', label: 'Barry Bonds' }],
    },
  ),
  evMinor(
    'sf-giants-2009-posey-debut',
    2009,
    '09-11',
    'Buster Posey MLB debut',
    'Player',
    'The catcher who would anchor three championships steps on a big-league field.',
    { related: [...team, { kind: 'person', label: 'Buster Posey' }] },
  ),
  evMinor(
    'sf-giants-2010-lincecum-back-to-back',
    2010,
    '11-01',
    'Tim Lincecum repeats as Cy Young–era ace',
    'Pitching',
    'The Freak’s swing-and-miss stuff headlines the first San Francisco title run.',
    { related: [...team, { kind: 'person', label: 'Tim Lincecum' }] },
  ),
  ev(
    'sf-giants-2012-cain-perfect',
    2012,
    '06-13',
    'Matt Cain throws a perfect game',
    'Pitching',
    'All 27 outs — the first perfecto in Giants franchise history.',
    {
      importance: 3,
      subtype: 'perfect-game',
      related: [...team, { kind: 'person', label: 'Matt Cain' }],
    },
  ),
  evMinor(
    'sf-giants-2012-pence-trade',
    2012,
    '07-31',
    'Hunter Pence deadline trade',
    'Trade',
    'High socks and speeches fuel another even-year October.',
    { subtype: 'trade', related: [...team, { kind: 'person', label: 'Hunter Pence' }] },
  ),
  evMinor(
    'sf-giants-2014-morse-heroics',
    2014,
    '10-16',
    'Michael Morse NLCS pinch heroics',
    'Postseason',
    'A swing off the bench helps push San Francisco toward a third crown.',
    { related: team },
  ),
  evMinor(
    'sf-giants-2016-last-even-year',
    2016,
    '10-02',
    'Final game of the even-year dynasty window',
    'Era',
    'MadBum, Posey, and Bochy close a remarkable half-decade of October baseball.',
    { related: [...team, { kind: 'person', label: 'Bruce Bochy' }] },
  ),
  evMinor(
    'sf-giants-2018-zaidi',
    2018,
    '11-07',
    'Farhan Zaidi hired as president of baseball ops',
    'Front office',
    'A modern analytics-forward regime begins reshaping the roster.',
    { related: team },
  ),
  evMinor(
    'sf-giants-2019-oracle-rename',
    2019,
    '01-09',
    'Oracle Park naming rights',
    'Ballpark',
    'The waterfront yard gets a new corporate identity; the cove stays sacred.',
    { subtype: 'venue', related: [...team, { kind: 'place', label: 'Oracle Park' }] },
  ),
  ev(
    'sf-giants-2021-bryant-trade',
    2021,
    '07-30',
    'Kris Bryant acquired at the deadline',
    'Trade',
    'Former MVP joins the 107-win club for a postseason push.',
    { importance: 2, subtype: 'trade', related: [...team, { kind: 'person', label: 'Kris Bryant' }] },
  ),
  evMinor(
    'sf-giants-2022-rodón-season',
    2022,
    '04-09',
    'Carlos Rodón’s electric strikeout season',
    'Pitching',
    'Left-handed heat leads the staff during a competitive West race.',
    { related: team },
  ),
]

/** Step 3+: SF Giants — structured for hover preview + rich detail (media/APIs later). */
export const sfGiantsSeedEvents: TimelineEvent[] = [
  {
    id: 'sf-giants-1958-move',
    title: 'Giants franchise moves to San Francisco',
    year: 1958,
    dateStart: '1958-03-03',
    category: 'Franchise',
    importance: 3,
    summary:
      'The New York Giants relocate west, establishing Major League Baseball in San Francisco.',
    description:
      'Owner Horace Stoneham’s decision brought National League baseball to the Bay Area alongside the Dodgers’ move to Los Angeles. Seals Stadium hosted the club before Candlestick Park opened.',
    media: [...heroMedia('1958'), ...galleryStub('1958')],
    facts: [
      { label: 'Previous city', value: 'New York' },
      { label: 'First NL season (SF)', value: '1958' },
    ],
    sources: [
      { title: 'MLB franchise history (placeholder)' },
      { title: 'SABR BioProject (placeholder)' },
    ],
    related: [
      ...team,
      { kind: 'place', label: 'Seals Stadium' },
    ],
  },
  {
    id: 'sf-giants-1962-pennant',
    title: 'First San Francisco pennant',
    year: 1962,
    dateStart: '1962-10-03',
    category: 'Postseason',
    importance: 2,
    summary:
      'The Giants win the NL pennant in their fifth California season, reaching the World Series.',
    description:
      'Willie Mays, McCovey, and Marichal anchored a tight pennant race that ended with San Francisco facing the Yankees in the Fall Classic.',
    media: [...heroMedia('1962'), ...galleryStub('1962')],
    facts: [
      { label: 'World Series opponent', value: 'New York Yankees' },
      { label: 'Games (WS)', value: '7' },
    ],
    sources: [{ title: '1962 World Series summary (placeholder)' }],
    related: [...team, { kind: 'person', label: 'Willie Mays' }],
  },
  {
    id: 'sf-giants-1989-world-series',
    title: 'Bay Bridge World Series',
    year: 1989,
    dateStart: '1989-10-17',
    category: 'Postseason',
    importance: 3,
    summary:
      'San Francisco meets Oakland in the World Series; the Loma Prieta earthquake interrupts Game 3.',
    description:
      'A cross-bay matchup captivated the region before the October 17 earthquake postponed play and shifted the series’ tone. The A’s won the championship in a four-game sweep.',
    media: [...heroMedia('1989'), ...galleryStub('1989')],
    facts: [
      { label: 'Opponent', value: 'Oakland Athletics' },
      { label: 'Notable delay', value: 'Loma Prieta — Game 3' },
    ],
    sources: [{ title: 'USGS + MLB archives (placeholder)' }],
    related: [...team, { kind: 'place', label: 'Candlestick Park' }],
  },
  {
    id: 'sf-giants-2000-park',
    title: 'Pacific Bell Park opens',
    year: 2000,
    dateStart: '2000-04-11',
    category: 'Ballpark',
    importance: 3,
    summary:
      'The waterfront ballpark opens on China Basin, replacing Candlestick Park as the Giants’ home.',
    description:
      'Designed with the cove and McCovey Cove in play, the park became a blueprint for urban, views-first MLB venues and deepened the club’s bond with the city.',
    media: [...heroMedia('2000'), ...galleryStub('2000')],
    facts: [
      { label: 'Neighborhood', value: 'China Basin / Mission Bay' },
      { label: 'First game', value: 'April 11, 2000' },
    ],
    sources: [{ title: 'Ballpark digest (placeholder)' }],
    related: [...team, { kind: 'place', label: 'McCovey Cove' }],
  },
  {
    id: 'sf-giants-2002-pennant',
    title: 'National League champions',
    year: 2002,
    dateStart: '2002-10-21',
    category: 'Postseason',
    importance: 2,
    summary:
      'The Giants win the NL pennant and advance to the World Series behind Barry Bonds’ MVP season.',
    description:
      'Bonds posted one of the great offensive seasons in modern history; the Giants reached Game 7 of the World Series against Anaheim.',
    media: [...heroMedia('2002'), ...galleryStub('2002')],
    facts: [
      { label: 'WS opponent', value: 'Anaheim Angels' },
      { label: 'NLCS', value: 'Giants over Cardinals' },
    ],
    sources: [{ title: '2002 postseason stats (placeholder)' }],
    related: [...team, { kind: 'person', label: 'Barry Bonds' }],
  },
  {
    id: 'sf-giants-2010-championship',
    title: 'First title in San Francisco',
    year: 2010,
    dateStart: '2010-11-01',
    category: 'Championship',
    importance: 3,
    summary:
      'San Francisco wins its first World Series as Edgar Rentería’s clutch hit seals Game 5 in Texas.',
    description:
      'Pitching, defense, and timely hitting defined the postseason. Rentería earned Series MVP as the Giants closed out the Rangers on the road.',
    media: [...heroMedia('2010'), ...galleryStub('2010')],
    facts: [
      { label: 'WS MVP', value: 'Edgar Rentería' },
      { label: 'Manager', value: 'Bruce Bochy' },
    ],
    sources: [{ title: '2010 World Series film (placeholder)' }],
    related: [...team, { kind: 'person', label: 'Tim Lincecum' }],
  },
  {
    id: 'sf-giants-2012-championship',
    title: 'Second championship in three years',
    year: 2012,
    dateStart: '2012-10-28',
    category: 'Championship',
    importance: 3,
    summary:
      'The Giants sweep the Tigers for their second title, anchored by dominant postseason pitching.',
    description:
      'Scutaro’s NLCS heroics and a locked-in rotation carried October momentum into a decisive World Series performance against Detroit.',
    media: [...heroMedia('2012'), ...galleryStub('2012')],
    facts: [
      { label: 'NLCS MVP', value: 'Marco Scutaro' },
      { label: 'WS result', value: 'Giants 4–0' },
    ],
    sources: [{ title: 'Postseason WPA charts (placeholder)' }],
    related: [...team, { kind: 'person', label: 'Buster Posey' }],
  },
  {
    id: 'sf-giants-2014-championship',
    title: 'Third championship in five seasons',
    year: 2014,
    dateStart: '2014-10-29',
    category: 'Championship',
    importance: 3,
    summary:
      'Madison Bumgarner’s historic October carries San Francisco past Kansas City in seven games.',
    description:
      'Bumgarner’s extended relief outing in Game 7 capped a postseason for the ages and cemented a dynasty-era identity for the even-year Giants.',
    media: [...heroMedia('2014'), ...galleryStub('2014')],
    facts: [
      { label: 'WS MVP', value: 'Madison Bumgarner' },
      { label: 'Games', value: '7' },
    ],
    sources: [{ title: 'Game 7 pitch chart (placeholder)' }],
    related: [...team, { kind: 'person', label: 'Madison Bumgarner' }],
  },
  {
    id: 'sf-giants-2021-record',
    title: 'Franchise-record 107 wins',
    year: 2021,
    dateStart: '2021-10-03',
    category: 'Regular season',
    importance: 2,
    summary:
      'The Giants set a franchise win record but exit in the NLDS after a tight division race.',
    description:
      'A veteran core and platoons maximized matchups in a 162-game sprint with the Dodgers; the postseason ended quickly against a division rival.',
    media: [...heroMedia('2021'), ...galleryStub('2021')],
    facts: [
      { label: 'Regular-season wins', value: '107' },
      { label: 'Division', value: 'NL West' },
    ],
    sources: [{ title: 'FanGraphs team page (placeholder)' }],
    related: [...team, { kind: 'place', label: 'Oracle Park' }],
  },
  ...SF_GIANTS_EXPANDED,
]
